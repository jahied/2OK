...
try:
    rq = requests.get('https://raw.githubusercontent.com/jahied/EC-170/main/Ac.txt').text
...
def menu():
    ...
    psb('         %s》%s》%s》%sWELLCOME TO BH-TEAM TOOL%s《%s《%s《'%(M,H,B,H,B,H,M))
    print('%s '%(O))
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    if fl.get("id") in files_cp:pass
                        ...
                        h_cp = "\r %s[%sBHT-CP%s] %s • %s%s%s          "%(K,P,K,fl.get("id"),i,ttll,tahun(fl.get("id")));cek_opsi(fl.get("id"),i,h_cp)
                    ...
                    if fl.get("id") in files_ok:pass
                        ...
                        else:print("\r %s[%sBHT-OK%s] %s • %s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
...
def result():
    ...
    if ch in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        ...
        try:
            okl = os.listdir("BHT-OK");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN OK FILE %s]'%(H,P,H));print('%s '%(O))
            ...
            try:
                ppp = _azimvau_dapunta_("BHT-OK/%s"%(files)).read().splitlines()
        ...
        try:
            cpl = os.listdir("BHT-CP");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN CP FILES %s]'%(H,P,H));print('%s '%(O))
            for file in cpl:print('%s [%s•%s] %s%s'%(H,P,H,K,file))
            print('%s '%(O));files = _cici_azimvau_('%s [%s•%s] %sENTER FILE NAME : '%(H,P,H,K));print('')
            if files == "":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));result()
            try:
                ppp = _azimvau_dapunta_("BHT-CP/%s"%(files)).read().splitlines()
...
def cek_result():
    ...
    for memek in buka_baju:
        ...
        _format_ = "\r %s[%sBHT-CP%s] %s          "%(K,P,K,kontol)
...
def genkey():
	try:
		...
		print("\033[0;93m   TOOL PRICE : %s350TK "%(H))
		...
		banner()
		psb('       %s》%s》%s》%sPOWER OF BH-TEAM%s《%s《%s《'%(M,H,B,H,B,H,M))
		print("\n\033[0;91m              YOU ARE NOT PREMIUM USER ")
		print("\033[0;91m           FIRST BUY PREMIUM KEY FROM OWNER")
		print("\033[0;91m.          PAYMENT CANT BE UNDONE")
		print("\033[0;91m.          CHANGE USER AGENT IF CP IDS CAME")
